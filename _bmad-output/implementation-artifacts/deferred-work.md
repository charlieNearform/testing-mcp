...
## Testing infrastructure, follow-up (2026-07-20)

- source: adding `test/epic-8-e2e-smoke.test.ts` (real-vitest-worker e2e coverage for Epic 8)
  summary: The 2026-07-15 mitigation for `test/watch.test.ts`'s full-suite-parallel-load flake (poll timeout raised to 150s) is not always sufficient — this session observed `pnpm test` intermittently still failing that same "poll timed out" assertion under full-suite load, both with and without the new e2e file present (baseline reproduced independently too, not solely caused by the new file). An early 5-real-vitest-fork draft of the new file made it fail reliably (3/3 runs); cutting to 3 forks measurably reduced but did not eliminate the residual risk (clean 3/3, then failed once on a subsequent run, matching the SAME pre-existing failure class, not a new one).
  evidence: Isolated via repeated full-suite runs with the new file present vs. renamed out of the way (both configurations showed at least one failure across ~5-6 total runs each, on this same machine/session). This confirms `deferred-work.md`'s 2026-07-15 note ("revisit if the timeout bump proves insufficient") — it has proven insufficient at least under this session's load conditions. The root-cause fix suggested there (give worker-forking integration tests their own non-parallel Vitest pool / lower `maxWorkers` for them) remains the real fix; every new real-worker-forking test file (this one included) adds more to the same shared contention pool in the meantime.
...
## Deferred from: code review of spec-unbounded-worker-timeout-and-bridge-resilience (2026-07-16)

- source_spec: `spec-unbounded-worker-timeout-and-bridge-resilience.md`
  summary: Raising the mcp-bridge's SSE `maxRetries` to 200 has an unweighed downside for a *deliberate* `test-mcp stop` (no restart coming) while a bridge is attached: the reconnect loop now spends ~90+ minutes (capped exponential backoff across 200 attempts) retrying and logging `daemon transport error` before giving up, versus the near-instant giveup before this change. Relatedly, there's no circuit breaker for a genuinely-down daemon: if `recreateSession()` itself fails (connection refused, not a 404), every subsequent client message independently re-triggers its own fresh `recreateSession()` attempt with no minimum interval between attempts — a reconnection storm hammering an unreachable daemon once per incoming message rather than backing off as a unit.
  evidence: MEDIUM. Real trade-off, but distinguishing "transient blip in a live daemon" from "daemon deliberately stopped or fully gone" would need either a clean-shutdown signal the SDK doesn't expose today or a bridge-level circuit breaker (e.g. a shared backoff/cooldown across `recreateSession` attempts, distinct from the SDK's own per-stream reconnect budget). Out of scope for the reported 15-20 minute long-run scenario, which this diff already fixes.
- source_spec: `spec-unbounded-worker-timeout-and-bridge-resilience.md`
  summary: The bridge's 404-only recovery path doesn't cover a daemon restart that also rotates the bearer token — that would 401, not 404, and per the current design every other error type keeps today's log-only (no recovery) behavior.
  evidence: LOW. The token is documented and designed to be stable across restarts (persisted in `~/.test-mcp/config.json`, see the field comment in `src/daemon/index.ts`), so this only matters if an operator manually rotates/deletes the token or overrides `TEST_MCP_TOKEN` mid-session — a deliberate action outside the "session lost mid-run" scenario this spec targets. A 401 also surfaces distinctly in the logs (not a silent black hole), which softens the impact even without automatic recovery.
- source_spec: `spec-unbounded-worker-timeout-and-bridge-resilience.md`
  summary: Concurrent/out-of-order message handling around `recreateSession()` isn't fully hardened: a message sent against the transport that was current when its own send began can still be mid-flight when a DIFFERENT message's 404 triggers recreation; if that in-flight send's rejection arrives after `clientTransport` has already been reassigned, it independently re-triggers ANOTHER `recreateSession()` call (discarding the still-good just-created transport) rather than recognizing itself as a stale/superseded failure.
  evidence: LOW-MEDIUM. Narrower than it sounds — requires genuinely overlapping in-flight sends spanning a recreation boundary — and the worst consequence (a stale transport spuriously killing the whole bridge later) is already closed by this story's stale-transport-cleanup fix. A generation/version counter on `clientTransport` (skip recreating if the transport has already changed since this failure's send began) would close the remaining redundant-recreation gap; no test currently exercises this interleaving.
- source_spec: `spec-unbounded-worker-timeout-and-bridge-resilience.md`
  summary: The end-to-end bridge-recovery test (`test/cli-mcp-bridge.test.ts`) only proves recovery for a session lost BETWEEN two separate tool calls (daemon restarted while idle). It does not exercise the harder, originally-reported scenario: the session dying WHILE a long `run_tests` call is actually in flight, mid-execution.
  evidence: MEDIUM. The SDK's own SSE reconnection (with resumption tokens/Last-Event-ID replay), strengthened by this story's raised `maxRetries`, is the primary defense for that specific in-flight scenario — this story's new 404-recreate-session logic is a secondary fallback for when the session itself (not just the stream) is gone. Worth a dedicated integration test that kills/restarts the daemon partway through a real long-running worker execution and confirms the client still eventually receives that call's result, to validate the two mechanisms actually compose correctly.

## Deferred from: test/epic-8-e2e-smoke.test.ts (2026-07-20)

- source_spec: `code-review-epic-8-async-execution-observability-2026-07-20.md`
  summary: `get_test_status(runId)`'s queued-disambiguation fix (this session's resolved decision 2) only distinguishes "not-yet-started" from "the run occupying the project's slot" -- it does not distinguish a run that already finished from one that's merely queued. Polling for an OLDER run's `runId` AFTER a NEWER run for the same project has been promoted (queue advances the instant the older one settles) incorrectly reports `{state:"queued"}` for the older run instead of its actual completed result, because `RunStatus`/`runState` holds only one current entry per project, overwritten on each promotion.
  evidence: Found writing `test/epic-8-e2e-smoke.test.ts`: an early draft polled run A's own `runId` to completion after concurrently firing run B for the same project; it timed out at `{state:"queued"}` forever, because B's promotion overwrote `runState` for the project the instant A settled, before the poll could observe A's own "complete" state. Not a realistic failure mode for a well-behaved caller (poll your OWN run's `runId` without firing a second one first), but a real gap if an agent ever does overlap calls and then goes back to check on the earlier one. Fix would need per-runId (not per-project) terminal-state retention, e.g. keeping the last N completed `RunStatus` entries by `runId` rather than a single current one.

## Noted during: code review of epic-8-async-execution-observability (2026-07-20)

- **Pre-existing test flake, unrelated to Epic 8**: `pnpm test` intermittently fails a `total`-count assertion in `test/worker-run.test.ts` and/or `test/plan-commit.test.ts` (e.g. expecting 2, getting 3) when the full suite runs, but both pass reliably in isolation. Both share `test-fixtures/sample-project` — reproduced on a clean pre-Epic-8 checkout too (confirmed via `git stash`), so this is not caused by this review's patches. Likely parallel test-file contention over that shared fixture directory (e.g. a coverage map or cache file written by one file's run being read mid-write by another). Worth a dedicated investigation/hermetic-fixture-per-file fix.

## Deferred from: code review of epic-8-async-execution-observability (2026-07-20)

- Missing regression test for Story 8.5's own testing note: no test asserts that log-only worker activity (stdout/stderr, no test-level progress) does not reset/block the stall watchdog. Production code path (`appendLog`, `src/orchestrator/index.ts`) looks correct — never calls `touchLiveProgress` — but the mandated guard test was never written (`test/orchestrator-stall-watchdog.test.ts`).
- Story 8.3's testing note ("`startDaemon()` threads `staleTestGraceMs` into the constructed `Orchestrator`") isn't asserted end-to-end in `test/daemon.test.ts` — only the config defaults themselves are tested; the wiring code itself is correct (`src/daemon/index.ts`).
